# AdaptLearn — Spaced Repetition Learning Engine v1.5
...
Adaptive quiz and flashcard system with SM-2 scheduling, difficulty adjustment, mastery tracking, and performance analytics.
...
function main() {
  ...
  for (let i = 0; i < Math.min(quiz.questions.length, simulatedAnswers.length); i++) {
    ...
    console.log(`    Next review in ${result.interval} day(s) | +${result.xpEarned} XP`);
    if (result.newBadges.length > 0) {
      result.newBadges.forEach(b => console.log(`    🏅 NEW BADGE: ${b.name} — ${b.description}`));
    }
  }
  ...
  console.log(`  ⭐ Level ${report.learner.level} (${report.learner.levelProgress})`);
  ...
  console.log(`  💰 Total XP: ${report.learner.totalXP}`);

  console.log(`\n  🏅 Badges:`);
  report.badges.forEach(b => {
    console.log(`     ${b.earned ? '✅' : '⬜'} ${b.name} — ${b.description}`);
  });
  ...
  console.log(`\n  🔓 Topic Access:`);
  report.topicAccess.unlocked.forEach(t => console.log(`     ✅ ${t} — unlocked`));
  report.topicAccess.locked.forEach(t => console.log(`     🔒 ${t} — requires: ${t.requires.join(', ')}`));
